...
import SAMLRequestPage from '../../../containers/SAMLRequestPage.jsx';
const styles = theme => ({
  root: {
    textAlign: 'center',
...
class SAMLCardPanelPage extends React.Component {
  ...
  async processAuth(event) {
    ...
    try {
      ...
      if (
        ...
        response.status === 200 &&
        response.redirected === false &&
        this.authParms.authScheme1 === 'SAML'
      ) {
        const resData = await response.json();
        console.log(
  ...
  render() {
    ...
    if (this.authParms.authScheme1 === 'SAML' && this.isRedirected) {
...
SAMLCardPanelPage.contextTypes = {
  router: PropTypes.object.isRequired
};

SAMLCardPanelPage.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withRoot(withStyles(styles)(SAMLCardPanelPage));
