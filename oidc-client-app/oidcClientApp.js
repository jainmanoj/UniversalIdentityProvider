...
module.exports = (issuer) => {
  ...
  app.use(async (ctx, next) => {
    if (!ctx.session.client && !ctx.path.startsWith('/samltest') && !ctx.path.startsWith('/setup') && !ctx.path.endsWith('/jwks.json')) {
      ctx.redirect('/setup');
    }
    await next();
  });
  ...
  router.get('/samltest', async (ctx, next) => {
    await ctx.render('samltest', { session: ctx.session });
    return next();
  });

  // THis is important to handle preflight cors error due to cross origin
  // We need to have some configuration for handle configration of cores
  // Make sure to set credentials = true options, otherwise cookies will not be set.
